...
module.exports.readPost = (req, res) => {
  PostModel.find()
    ...
    .then((findPosts) => {
      res
        .status(200)
        .json({ requestById: req.user.data._id, allPosts: findPosts });
    })
...
module.exports.createPost = (req, res) => {
  ...
  const newPost = new PostModel({
    ...
    picture:
      req.file != null
        ? `https://${req.get("host")}/portfolio/groupomania/images/post/${
            req.file.filename
          }`
  ...
  newPost
    ...
    .then((postCreated) => {
      res
        .status(200)
        .json({ requestById: req.user.data._id, createdPost: postCreated });
    })
...
module.exports.updatePost = (req, res) => {
  ...
  PostModel.findById(req.params.id)
    .then((post) => {
      ...
      if (
        postedBy === connectedUser ||
        connectedUser === "62f2ae7fd2fc5c1305443984"
      ) {
        const updatedRecord = {
          message: req.body.message,
          picture:
          ...
          if (
            ...
            PostModel.findByIdAndRemove(req.params.id, (err, docs) => {
              if (!err)
                res
                  .status(200)
                  .json({ requestById: req.user.data._id, deletedPost: docs });
