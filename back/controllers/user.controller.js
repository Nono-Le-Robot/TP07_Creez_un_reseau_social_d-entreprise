...
module.exports.updateUser = (req, res) => {
  const updatedRecord = {
    picture:
      req.file != null
        ? `https://${req.get("host")}/portfolio/groupomania/images/profil/${
            req.file.filename
          }`
        : "https://sannier-renaud.fr/portfolio/groupomania/images/default/default.jpg",
  };
  UserModel.findById(req.params.id)
    .then((user) => {
      ...
      if (
        userToModify === connectedUser ||
        connectedUser === "62f2ae7fd2fc5c1305443984"
      ) {
        UserModel.findByIdAndUpdate(req.params.id, { $set: updatedRecord })
          .then(() => {
            ...
            $set: {
              picture:
                req.file != null
                  ? `https://${req.get(
                      "host"
                    )}/portfolio/groupomania/images/profil/${req.file.filename}`
